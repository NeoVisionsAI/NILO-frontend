...
export function PainScalePanel({ scales, onChange }: PainScalePanelProps) {
  ...
  return (
    <section className="nilo-pscale" aria-label="Escalas de dolor">
      ...
      <div className="nilo-pscale__grid">
        <div className="nilo-pscale__form">
        ...
        <div className="nilo-pscale__list-panel">
          <p className="nilo-pscale__list-title">
            Registradas
            {scales.length > 0 && <span className="nilo-pscale__list-count">{scales.length}</span>}
          </p>
          <div className="nilo-pscale__list m3-scroll">
            {scales.length === 0 ? (
              <p className="nilo-pscale__empty">Sin escalas añadidas.</p>
            ) : (
              scales.map((scale) => (
